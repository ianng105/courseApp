...
<div class="container">
    ...
    <sec:authorize access="isAuthenticated()">
        <form action="<c:url value='/poll/${poll.poll_id}/vote'/>" method="post">
            <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
            <div class="poll-options">
                <c:forEach var="choice" items="${choices}">
                    <label class="option-label">
                        <input type="radio" name="choiceId" value="${choice.id}" ${selectedChoiceId eq choice.id ? 'checked' : ''} required>
                            ${choice.choice}
                    </label>
                </c:forEach>
            </div>
            <button type="submit" class="submit-btn">${hasVoted ? 'Update Vote' : 'Submit Vote'}</button>
        </form>
    </sec:authorize>
    <sec:authorize access="isAnonymous()">
        <p style="text-align: center; color: #94a3b8;">Please <a href="<c:url value='/login'/>" style="color: #38bdf8;">log in</a> to vote.</p>
    </sec:authorize>

    <h2>Current Results</h2>
    <div class="poll-results">
        <c:forEach var="choice" items="${choices}">
            <p>${choice.choice} (${choice.voted} votes)</p>
            <div class="result-bar">
                <div class="result-bar-inner" style="width: ${choice.voted * 10}%;">
                    ${choice.voted} votes
                </div>
            </div>
        </c:forEach>
    ...
    <div class="comment-section">
        ...
        <sec:authorize access="isAuthenticated()">
            <form action="<c:url value='/poll/${poll.poll_id}/comment'/>" method="post" style="margin-top: 2rem;">
                <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
                <textarea name="content" placeholder="Write your comment here..." required></textarea>
                <button type="submit" class="submit-btn" style="width: auto;">Post Comment</button>
            </form>
        </sec:authorize>
    </div>
</div>
